...
See `CONTRIBUTING.md` for architecture guidelines and the feature-grouped layout of `src/`.
...
- Never use `.js` extensions in internal/relative imports. Use extensionless paths: `from "./config/hook"` not `from "./config/hook.js"`. (Some external packages with strict subpath `exports` maps require `.js`; that's fine and unavoidable.)
